# -*- coding: utf-8 -*-
"""
Created on Mon Jan 14 10:44:15 2019
...
def ins():
    ...
    def ex():
        ...
        dob=dobEntered.get()
        doj=dojEntered.get()
        fatn=FathernameEntered.get()
        motn=MothernameEntered.get()
        sibn=SiblingsEntered.get()
        fid=F1.get()
        qu=Q1.get()
        po=P1.get()
        st=S1.get()
        ...
        cur=conn.cursor()
        cur.execute('CREATE TABLE IF NOT EXISTS STUDENTS (FNAME TEXT,LNAME TEXT, BRANCH TEXT, SEMESTER INT, ADMNO INT, PERMANANTADD TEXT,PRESENTADD TEXT, CGPA INT, CLASSROLL INT,DOB INT,DOJ INT)')
        ...
        cur.execute("CREATE TABLE IF NOT EXISTS FAMILY(FATHERNAME TEXT, MOTHERNAME TEXT, SIBLINGS TEXT)")
        cur.execute("INSERT INTO FAMILY(FATHERNAME, MOTHERNAME, SIBLINGS) VALUES(?,?,?)", (fatn, motn, sibn))
        cur.execute("CREATE TABLE IF NOT EXISTS FACULTY(FACID INT,QUALIFICATION TEXT, POSITION TEXT, STUDID INT)")
